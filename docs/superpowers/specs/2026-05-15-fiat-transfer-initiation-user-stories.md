...
**Phase 1 (UC1 + UC2):** US-01 → US-10  
**Phase 2 (UC5):** US-11 → US-15  
**Phase 3 (UC3 + UC4):** US-16 → US-21
...
- [ ] Checker can also **Reject** from HexAdmin:
  ...
  - Slack notification sent to ops team and trading team (see US-09)
...
- [ ] Confirming Retry initiates a **fresh maker-checker cycle** (same flow as initiating from scratch)
- [ ] The existing Manual Fiat Confirmation action remains visible and accessible at all times
...
### US-09 — Slack Notifications for UC1 / UC2 Transfer Events

**As an** Ops team member or Trader,  
**I want to** receive a Slack notification when a CL2 or ML1 fiat transfer is rejected or reaches a terminal failure,  
...
- [ ] Notification includes: trade ID, amount, currency, failure reason

---

### US-10 — Audit Trail for UC1 / UC2 Fiat Transfers

**As a** Compliance Officer,  
**I want** every state transition on a CL2 / ML1 fiat transfer to be logged with full details,  
...
| Event | Data captured |
|---|---|
| Initiated | Operator ID, timestamp, trade ID, leg (CL2/ML1), from/to accounts, amount, currency, transfer method |
...
### US-11 — Initiate House-to-House Bank Transfer
...
- [ ] Maker submits → request appears in the Fiat Transfers sub-tab under Pending Requests with status **Pending Approval**
- [ ] Maker cannot approve their own submission

---

### US-12 — Approve House-to-House Transfer as Checker
...
- [ ] Same approval flow as US-05 with the following differences:
  - Mobile template shows Transfer Reference (UUID) instead of Trade ID
...
- [ ] On rejection: transfer reverts; Slack notification sent to ops team and trading team (see US-14)
- [ ] Failure handling: same as US-07 (auto-retry → Transfer Failed → fresh maker-checker cycle)

---

### US-13 — Risk Controls for House-to-House Transfer
...
### US-14 — Slack Notifications for UC5 Transfer Events

**As an** Ops team member or Trader,  
**I want to** receive a Slack notification when a house-to-house transfer is rejected or fails,  
**so that** I can take action without continuously monitoring HexAdmin.
...
- [ ] Slack notification sent to **both** the ops team channel and the trading team channel for:
  ...
  - Any other terminal failure (immediately)
- [ ] No Slack notification is sent for transient failures where automatic retry is still pending
- [ ] Notification includes: transfer reference (UUID), amount, currency, failure reason

---

### US-15 — Audit Trail for UC5 House-to-House Transfers

**As a** Compliance Officer,  
**I want** every state transition on a house-to-house transfer to be logged with full details,  
**so that** there is a complete audit trail for internal fund movements.
...
| Event | Data captured |
|---|---|
| Initiated | Operator ID, timestamp, transfer reference (UUID), from/to accounts, amount, currency, transfer method |
| Approved / Rejected | Checker ID, timestamp |
| Bank API called | Timestamp, API request reference |
| Bank confirmed | Bank transaction ID, value date, timestamp |
| Failed | Failure reason, retry count |

- [ ] Bank transaction IDs are stored and visible in the transfer detail view
...
### US-16 — View Incoming Bank Transactions on CL1 Trade
...
### US-17 — Confirm Incoming Fiat Settlement: Client Pays Hex (CL1)
...
### US-18 — Transaction Amount Tracking for Incoming Fiat
...
### US-19 — Confirm Incoming Fiat Settlement: LP Pays Hex (ML2)
...
- [ ] Same flow and acceptance criteria as US-17, with the following differences:
...
### US-20 — Unmatched Incoming Transactions View *(lower priority)*
...
### US-21 — Audit Trail for UC3 / UC4 Incoming Fiat Confirmations

**As a** Compliance Officer,  
**I want** every incoming fiat confirmation action to be logged,  
**so that** there is a record of which transactions were matched to which trade and by whom.
...
| Event | Data captured |
|---|---|
| Confirmed | Operator ID, timestamp, trade ID, leg (CL1/ML2), selected transaction references, amounts used, value dates |

- [ ] Recorded bank transaction references and value dates are visible in the trade detail view
